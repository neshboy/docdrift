...
function extractPathTokens(text3, bareExtensions) {
...
var remarkParsePlugin = typeof remarkParse === "function" ? remarkParse : remarkParse.default;
var processor = unified().use(remarkParsePlugin);
function parseMarkdown(text3) {
  return processor.parse(text3);
}
function extractReferences(text3, docFile, bareExtensions) {
  ...
  visit(tree, "code", (node2) => {
    ...
    const lang = (node2.lang ?? "").trim();
    if (lang === SYMBOL_BLOCK_INFO) {
      const lines = node2.value.split(/\r?\n/);
    ...
    const tokens = extractPathTokens(node2.value, bareExtensions);
...
function candidatePaths(rawPath, absoluteDocFile, root) {
  ...
  candidates.push(import_node_path2.default.resolve(import_node_path2.default.dirname(absoluteDocFile), rawPath));
  candidates.push(import_node_path2.default.resolve(root, rootRelative));
...
async function listSourceFiles(opts) {
  ...
  const files = relFiles.map((f) => import_node_path3.default.resolve(opts.root, f));
...
async function findSymbolDeclaration(name, opts) {
  ...
  for (const file of files) {
    let content3;
    try {
      content3 = import_node_fs2.default.readFileSync(file, "utf8");
    } catch {
      continue;
    }
...
async function runCheck(options) {
  const root = import_node_path4.default.resolve(options.root);
  ...
  for (const docFileRel of docFilesRel) {
    const absoluteDocFile = import_node_path4.default.resolve(root, docFileRel);
    const text3 = import_node_fs3.default.readFileSync(absoluteDocFile, "utf8");
    const references = extractReferences(text3, docFileRel, DEFAULT_PATH_EXTENSIONS);
    for (const reference of references) {
      ...
      if (match) {
        findings.push({
          reference,
          ok: true,
          matchedAt: { file: import_node_path4.default.relative(root, match.file), line: match.line }
        });
      } else {
        findings.push({
          reference,
